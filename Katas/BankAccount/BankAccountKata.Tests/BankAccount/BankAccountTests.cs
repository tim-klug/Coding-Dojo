...
namespace BankAccountKata.Tests.BankAccount
{
	...
	public class BankAccountTests
	{
		...
		[Test(Description = "Given a bank account with a transaction history, when the account holder asked for the account movements, then all movements are displayed one by one.")]
		public void Given_a_bank_account_with_a_transaction_history_when_the_account_holder_asked_for_the_account_movements_then_all_movements_are_displayed_one_by_one()
		{
			var account = Substitute.For<IAccount>();
			account.TransactionHistory().Returns(new List<ITransaction>
			{
//				new Transaction
//				{
//					Amount = 10,
//					FromAccount = Substitute.For<IAccount>(),
...
//				}
			...
			var result = account.TransactionHistory();
